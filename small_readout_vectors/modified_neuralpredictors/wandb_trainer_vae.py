...
def standard_trainer(
    ...
    **kwargs
    ...
    def get_kld_loss(model, dataloader):
        sum_kld_loss = 0
        for key in dataloader.keys():
            mu, logvar = model.readout[key].autoencoder.return_params()
            kld_loss = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
            sum_kld_loss += kld_loss
    ...
    def full_objective(model, dataloader, data_key, neuron_idcs, *args, **kwargs):
        ...
        mu, logvar = model.readout[data_key].autoencoder.return_params()
        kld_loss = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
        kld_loss /= vae_latent_dim

        return poisson_weight * poisson_loss + kld_weight * kld_loss
