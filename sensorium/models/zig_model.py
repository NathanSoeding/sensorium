...
from .readouts import MultipleGeneralizedFullGaussian2d, MultipleGeneralizedFactorized2d
...
def stacked_core_zig_factorized_readout(
    dataloaders,
    seed,
    gamma_params_dir,
    hidden_channels=32,
    ...
    """
    Model class of a Stacked2dCore (from neuralpredictors) and a GeneralizedFactorized2d
    readout with 2 image-dependent outputs (q, theta), fused into a ZIGEncoder. `loc` and `k` are
    estimated per neuron from data (see sensorium/utility/gamma_params_from_data.py) and held frozen
    (not fine-tuned).
    ...
    Args:
        ...
        gamma_params_dir: directory containing one `<data_key>_gamma_params.npz` file per session,
            produced by sensorium/utility/gamma_params_from_data.py
        all other args: See Documentation of Stacked2dCore in neuralpredictors.layers.cores and
            GeneralizedFactorized2d in neuralpredictors.layers.readouts
    ...
    readout = MultipleGeneralizedFactorized2d(
        in_shape_dict=in_shapes_dict,
        loader=dataloaders,
        n_neurons_dict=n_neurons_dict,
        bias=readout_bias,
