...
def static_loader(
    ...
    # Modified neuron seleciton code

    num_neurons = len(dat.neurons.unit_ids)

    if exclude_neuron_n is None:
        exclude_neuron_n = 0
    if neuron_n is None:
        neuron_n = num_neurons - exclude_neuron_n

    assert (
        num_neurons >= exclude_neuron_n + neuron_n
    ...
    shuffled_ids = np.arange(num_neurons, dtype=int)
    np.random.shuffle(shuffled_ids)
    neuron_ids = shuffled_ids[exclude_neuron_n:(exclude_neuron_n + neuron_n)]
        
    np.random.set_state(random_state)

    more_transforms = [Subsample(neuron_ids), ToTensor(cuda)]
