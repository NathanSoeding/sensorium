...
from .readouts import MultipleFullGaussian2d, MultipleFactorized2d, MultipleGaussianRetinaMean
...
def stacked_core_gaussian_retina_mean_readout(
    dataloaders,
    seed,
    hidden_channels=32,
    input_kern=13,
    ...
    """
    Model class of a stacked2dCore (from neuralpredictors) and a GaussianRetinaMean readout, whose
    per-neuron RF center is predicted from the neuron's cortical (x, y) position (z dropped) via a
    linear layer shared across neurons of a session, and whose per-neuron 2d covariance is learned
    directly.
    ...
    Args:
        ...
        seed: random seed
        all other args: See Documentation of Stacked2dCore in neuralpredictors.layers.cores and
            GaussianRetinaMean in neuralpredictors.layers.readouts
    ...
    grid_mean_predictor = {
        'type': 'cortex',
        'input_dimensions': 3,  # keep z around; GaussianRetinaMean drops it internally
        '_': '_',
    }
    grid_mean_predictor, grid_mean_predictor_type, source_grids = prepare_grid(grid_mean_predictor, dataloaders)
    ...
    readout = MultipleGaussianRetinaMean(
        in_shape_dict=in_shapes_dict,
        loader=dataloaders,
        n_neurons_dict=n_neurons_dict,
        bias=readout_bias,
