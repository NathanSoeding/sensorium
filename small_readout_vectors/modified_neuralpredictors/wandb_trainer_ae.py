...
def tracker_recon_loss(model, dataloaders, neuron_idcs=None):
    loss_sum = 0
    for key in dataloaders.keys():
        
        # Pass features through AE and get MSE loss
        with torch.no_grad():
            readout = model.readout[key]
            _ = readout.features
            loss = readout.autoencoder.get_recon_loss(neuron_idcs[key])
            loss_sum += loss / neuron_idcs[key].shape[0]  # Log loss per neuron to compare val and train losses
...
def standard_trainer(
    ...
    **kwargs
    ...
    def full_objective(model, dataloader, data_key, neuron_idcs, *args, **kwargs):
        ...
        poisson_loss = loss_scale * criterion(preds, targets) + regularizers
        recon_loss = (
            model
            .readout[data_key]
            .autoencoder
            .get_recon_loss(train_neurons[data_key])
        )

        #print(poisson_loss, recon_loss)
        total_loss = poisson_weight * poisson_loss + mse_weight * recon_loss
    ...
    optimizer = torch.optim.Adam([
        {'params': autoenc_params, 'lr': lr_init}, 
    ...
    if wandb_project:
        tracker_dict = dict(
            ...
            train_recon_loss=partial(
                tracker_recon_loss,
                model,
                dataloaders['validation'],
                neuron_idcs=train_neurons,
            ),
            validation_recon_loss=partial(
                tracker_recon_loss,
                model,
                dataloaders['validation'],
                neuron_idcs=validation_neurons,
